    ...
    *)  
    ...
    　if ［ "$secure"=1 ］  
    　then  
    wget --no-check-certificate -q -t 3 -T 3 \http://${hostname}:${port}/${status_page} -O ${output_dir}/nginx-status.1  
    sleep 1  
    wget --no-check-certificate -q -t 3 -T 3 \http://${hostname}:${port}/${status_page} -O ${output_dir}/nginx-status.2  
    　else  
    wget -q -t 3 -T 3 http://${hostname}:${port}/${status_page} \-O ${output_dir}/nginx-status.1  
    sleep 1  
    wget -q -t 3 -T 3 http://${hostname}:${port}/${status_page} \-O ${output_dir}/nginx-status.2  
    　fi  
